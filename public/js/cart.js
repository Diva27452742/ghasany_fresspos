...
function populateMemberDropdown() {
    ...
    if (sel) {
        ...
        members.forEach(m => {
            ...
            opt.value = m.name;
            opt.textContent = m.name + (m.discount_pct > 0 ? ` (Diskon ${m.discount_pct}%)` : '');
            opt.dataset.id = m.id;
            opt.dataset.discountPct = m.discount_pct || 0;
            opt.dataset.discountStatus = m.discount_status || 'Aktif';
            sel.appendChild(opt);
        });
...
// ============================================================
// BAGIAN KASIR: PILIHAN METODE PEMBAYARAN & TIPE PESANAN
// ============================================================
let selectedPaymentMethod = "Tunai"; // Default: Tunai
let selectedOrderType = "Dine In";   // Default: Dine In

const orderTypeButtons  = document.querySelectorAll('#orderTypeContainer button');
const customerNameInput = document.getElementById('customerNameInput');
const tableSeatInput    = document.getElementById('tableSeatInput');
const tableSeatGroup    = document.getElementById('tableSeatGroup');

orderTypeButtons.forEach(btn => {
    btn.addEventListener('click', () => {
        orderTypeButtons.forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        selectedOrderType = btn.getAttribute('data-order-type') || 'Dine In';

        if (tableSeatGroup) {
            if (selectedOrderType === 'Take Away') {
                tableSeatGroup.style.display = 'none';
                if (tableSeatInput) tableSeatInput.value = '';
            } else {
                tableSeatGroup.style.display = 'block';
            }
        }
    });
});
...
paymentButtons.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        btn.classList.add('active');
        selectedPaymentMethod = btn.getAttribute('data-method') || 'Tunai';
    });
});
...
btnCheckout.addEventListener('click', async () => {
    if (cartState.length > 0) {
        ...
        const customerName = customerNameInput ? (customerNameInput.value.trim() || 'Umum') : 'Umum';
        let tableSeat = '-';
        if (selectedOrderType === 'Take Away') {
            tableSeat = 'Bungkus / Take Away';
        } else {
            tableSeat = tableSeatInput ? (tableSeatInput.value.trim() || 'Meja Umum') : 'Meja Umum';
        }
        ...
        try {
            ...
            if (API_PATH) {
                const response = await fetch(API_PATH, {
                    ...
                    body: JSON.stringify({
                        ...
                        kasir:          adminNameEl ? adminNameEl.textContent : 'Admin Utama',
            ...
            // BAGIAN KASIR: Isi Header & Detail Struk Pembayaran
            ...
            if (document.getElementById('modalOrderType'))    document.getElementById('modalOrderType').textContent    = selectedOrderType;
            if (document.getElementById('modalCustomerName')) document.getElementById('modalCustomerName').textContent = customerName;
            
            const modalTableSeatRow = document.getElementById('modalTableSeatRow');
            if (document.getElementById('modalTableSeat')) {
                document.getElementById('modalTableSeat').textContent = tableSeat;
            }
            if (modalTableSeatRow) {
                if (selectedOrderType === 'Take Away') {
                    modalTableSeatRow.style.display = 'none';
                } else {
                    modalTableSeatRow.style.display = 'flex';
                }
            }
